...
function Todos({data, setData}) {
    ...
    const Fetch = async () => {
        setLoading(true);
        const todo = await axios.get("http://54.86.126.202:3500/todo")
    ...
    const moveTodo = async (fromIndex, toIndex) => {
        ...
        try {
            await axios.post("http://34.192.51.127:3500/rearrange-todos", {newOrder: updatedData})
            setData(updatedData);
        } catch (error) {
            console.error(error);
            setData(prevData);
    ...
    const clearComplete = async () => {
        try {
            let response = await axios.delete("http://34.192.51.127:3500/complete/todo")
