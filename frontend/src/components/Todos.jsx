...
function Todos({ data, setData }) {
    ...
    const Fetch = async () => {
        ...
        try {
            ...
            const todoItem = await todo.data;
            setLoading(false);
            return todoItem;
        } catch (error) {
            setLoading(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (fetchTodo) setData(fetchTodo);
        };
    ...
    const clearComplete = async () => {
        try {
            const response = await axios.delete("http://localhost:3500/complete/todo");
    ...
    const showComplete = async () => {
        try {
            let fetchTodo = await Fetch();
            if (fetchTodo) {
                let complete = fetchTodo.filter(item => item.isCompleted === true); // Use strict equality
                setData(complete);
    ...
    const showActive = async () => {
        let fetchTodo = await Fetch();
        if (fetchTodo) {
            let active = fetchTodo.filter(item => item.isCompleted !== true); // Use strict equality
            setData(active);
            ...
            <div className="list_todo">
                ...
                <div className="bottom">
                    ...
                    <ul className="bottom__section">
                        <li
                            className={`${showTodoItem === "all" && "active"}`} // Use strict equality
                            onClick={showAll}
                        >
                            All
                        </li>
                        <li
                            className={`${showTodoItem === "active" && "active"}`} // Use strict equality
                            onClick={showActive}
                        >
                            Active
                        </li>
                        <li
                            className={`${showTodoItem === "complete" && "active"}`} // Use strict equality
